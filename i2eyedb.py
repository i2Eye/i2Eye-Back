...
def get_all_patients():
    try:
        ...
        for patient_ID in patient_ID_list:
            ...
            answer_select_query = """SELECT question_id, answers FROM answer WHERE patient_id = {0}""".format(id)
            ...
            for answer in answers_list:
                ...
                print(answer_string)
                question_select_query = """ SELECT question FROM question WHERE question_id = {0}""".format(matching_question_id)
                cursor.execute(question_select_query)
                connection.commit()
                question_string = cursor.fetchall()[0][0] # It's abit weird here as fetchall returns [('question',)].
            ...
            array_select_query = """SELECT completed_station FROM patient WHERE patient_id = {0}""".format(id)
            cursor.execute(array_select_query)
            connection.commit()
            completed_stations_array = cursor.fetchall()[0][0] # Same reason for [0][0] as above.
            ...
            for index in range(1, len(completed_stations_array) + 1):
                completion_info = completed_stations_array[index - 1] # IDs in db start from 1, while array indices start from 0.
                station_select_query = """SELECT station_name FROM station WHERE station_id = {0} """.format(index)
...
#  {registration: [{ num: 1, question: “Name”, answer: “” }, 
#                  { num: 2, question: “NRIC No.”, answer: “” }, …] } - Maybe need to ask front end what data they want to pass - Rollie
    ...
    try:
        ...
        for station in data.keys():
            for json_question in data[station]:
                  ...
                  cursor2 = connection.cursor()
                  question_id_query = """SELECT question_id FROM question WHERE question = {0}""".format(question)
                  cursor2.execute(question_id_query)
                  question_id = cursor2.fetchall()

                  answer_update_query = """UPDATE answers FROM answer WHERE question_id = {0}""".format(question_id)
...
def insert_stuff_test_patch_4():
    # The commands I used to generate the output sent in the group - Wei Kit (5/8/20)
